...
import { getDpeStats, getDvfStats, searchCompanies, DpeStats, DvfStats } from "@/lib/api";
...
export default function Dashboard() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const [d91, d92, f91, f92, comps] = await Promise.allSettled([
          getDpeStats("91"),
          getDpeStats("92"),
          getDvfStats("91"),
          getDvfStats("92"),
  ...
  const KPI = [
    ...
    {
      label: "Transactions DVF 92",
      value: loading ? "—" : dvf92 ? dvf92.total_transactions : "—",
      sub: `Rénovation : ${dvf92?.indicateur_renovation ?? "—"}`,
      icon: TrendingUp,
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-brand-900/30 border border-brand-600/40 rounded-xl p-6 flex items-center justify-between">
        <div>
          <p className="text-white font-semibold">Prêt à lancer une analyse stratégique ?</p>
          <p className="text-slate-400 text-sm mt-0.5">
            L&apos;agent IA croise toutes ces données et produit des recommandations actionnables.
          </p>
        </div>
        <Link
          href="/analysis"
